...
public class SwapDialogueType : DialoguePresenterBase
{   
    [Header("Prefabs")]
    ...
    [SerializeField] RectTransform? messageContainer = null;
    ...
    public override async YarnTask RunLineAsync(LocalizedLine line, LineCancellationToken token)
    {
        ...
        if (line.Metadata.Contains("message"))
        {
            ...
            if ((AppLoaderManager.Instance.messagingAppPrefab != null) && (AppLoaderManager.Instance.messagingAppPrefab.activeSelf == false))
            {
                AppLoaderManager.Instance.messagingAppPrefab.SetActive(true);
            }
        ...
        else
        {
            ...
            if (autoAdvance)
            {
                await YarnTask.Delay((int)(autoAdvanceDelay * 1000), token.HurryUpToken).SuppressCancellationThrow();
            }
            else
            {
